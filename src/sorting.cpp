 ...
 */
  for (int i = 0; i < len; ++i) {
    int min_index = min_index_of_array(ar + i, len - i); // shrinks the search window as i progresses
    swap(ar + i, ar + i + min_index); // swaps to left w/out + i (focus on current index)
  }
}
